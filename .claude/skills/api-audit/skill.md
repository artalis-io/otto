...
The OTTO API manifesto requires that API logic be transport-agnostic. The core handler should be usable by HTTP (Keel), WASM, Unix sockets, or direct C calls.
...
If `{prefix}_api.h` doesn't exist, scaffold it from the Keel handlers:
...
For each unannotated endpoint found in the Keel handlers, add scaffold:
...
- [ ] Keel server uses handler, not inline logic
