/*
 * test_transport_direct.c - FuelWise's half of the transport leak detector.
 *
 * Drives fw_api_handle() through sh_transport_direct, with NO transport
 ...
 * module-specific, so this file sticks to FuelWise's own handler.
 *
...
/* The whole point: a plain ShApiHandler, no cast. If fw_api_handle ever stops
 * matching the shared signature this line fails to compile. */
static ShApiHandler g_handler = fw_api_handle;
                ...
                ShApiResponse *resp)
{
    ...
    req.path = path;
    req.query = query;
    /* NULL ctx: fw_api_handle documents it as valid for stateless endpoints. */
...
int main(void)
{
    ...
    printf("=== Transport: direct driver over fw_api_handle ===\n");
