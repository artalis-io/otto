...
function getSelectedText(wasm) {
    ...
    const ptr = wasm.cs_focused_text();
 ...
 *
 * @param {Object} wasm - WASM module with cs_* exports
 * @param {Object} options - Configuration options
 * @param {Function} [options.onGlobalShortcut] - Callback for app-specific shortcuts when unfocused
 * @returns {Function} Cleanup function to remove the event listener
 *
 * Required WASM exports:
 *   cs_key_down(keyCode, shift, ctrl) -> bool
 *   cs_key_char(charCode) -> bool
 *   cs_focused_id() -> uint32
 *
 * @example
...
export function setupKeyboardHandler(wasm, options = {}) {
    ...
    function handleKeyDown(e) {
        ...
        if (e.key === 'Tab') {
            if (wasm.cs_key_down(e.keyCode, e.shiftKey ? 1 : 0, e.ctrlKey ? 1 : 0)) {
                e.preventDefault();
            }
            return;
        }

        if (wasm.cs_focused_id() !== 0) {
    ...
    async function handleClipboard(e, wasm) {
        ...
        if (e.key === 'c' || e.key === 'x') {
            ...
            if (selectedText) {
                ...
                if (e.key === 'x') {
                    wasm.cs_key_down(8, 0, 0); // Backspace deletes selection
                }
            }
        } else if (e.key === 'v') {
            // Paste
