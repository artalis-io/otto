...
5. cc_frame_end(dt)          Update timers, consume pending input
...
```c
#define CC_WIDGET_STATE_CAPACITY 256  // Compile-time configurable
...
Each frame, focusable widgets call `cc_register_focusable(id)`:

```c
uint32_t focusables[CC_MAX_FOCUSABLES];  // 64 slots
int focusable_count;
```
...
```c
cc_focus_next()  // Tab: move to next registered focusable
cc_focus_prev()  // Shift+Tab: move to previous
```
...
When focus changes (`cc_focus(new_id)`):
- `focused_id` updates
...
```c
bool pending_click;   // cc_set_pending_click() on mousedown
...
Platform keydown
    ↓
cc_key_down(keyCode, shift, ctrl)
    ↓
Tab? → cc_focus_next/prev
    ↓
Has focused widget?
    ↓
...
```c
cc_key_char(charCode)  // ASCII 32-126
```
...
- Ctrl+V: Read clipboard, call `cc_key_char()` per character
...
cursor = cc_clay_x_to_cursor(text, len, x_offset, font_size);
```
...
|-----------|------|--------------|
| Widget state table | `CC_WIDGET_STATE_CAPACITY * sizeof(CcWidgetState)` | Yes |
| Focusables array | `CC_MAX_FOCUSABLES * sizeof(uint32_t)` | Yes |
| Global state | `sizeof(CcState)` | No |
