 ...
 *
 * Generic Clay boilerplate is in cc_clay.c (clay-components).
 ...
 * Application State
 *
 * Domain-specific state only. Generic Clay state is in cc_clay.c.
 * ============================================================================ */
...
static void render_zoom_controls(void) {
    CLAY(CLAY_ID("ZoomControls"), {
        ...
        if (cc_button(CC_ID("zoom_in"), "+", NULL).clicked) {
            g_app.map.zoom = cc_map_scroll(g_app.map.zoom, 1, 0, 19);
        }
...
static void render_layer_panel(void) {
    CLAY(CLAY_ID("LayerPanel"), {
        ...
        if (cc_button(CC_ID("layer_carto"), "Carto", g_app.panels.layer_type == 1 ? &sel : &def).clicked)
            g_app.panels.layer_type = 1;
        if (cc_button(CC_ID("layer_terrain"), "Terrain", g_app.panels.layer_type == 2 ? &sel : &def).clicked)
...
static void render_info_panel(void) {
    ...
    CLAY(CLAY_ID("InfoPanel"), {
        ...
        const CcInputStyle input_style = { .width = 180, .height = 28, .font_size = 12, .padding = 8, .corner_radius = 4 };
        cc_input(CC_ID("search"), g_app.text.search, &g_app.text.search_len,
...
static void render_tile_info(void) {
    ...
    int tile_x = (int)cc_map_lon_to_tile_x(g_app.map.lon, g_app.map.zoom);
    int tile_y = (int)cc_map_lat_to_tile_y(g_app.map.lat, g_app.map.zoom);
...
static void render_ui(void) {
    CLAY(CLAY_ID("Root"), {
        ...
        cc_map(g_app.map.component_id, &g_app.map.lat, &g_app.map.lon, &g_app.map.zoom,
...
EXPORT void map_init(int width, int height) {
    ...
    g_app.map.component_id = CC_ID("map");
...
EXPORT void map_set_zoom(int zoom) {
    g_app.map.zoom = cc_map_scroll(zoom, 0, 0, 19);
}
...
EXPORT void map_pointer_move(float x, float y) {
    bool dragging = cc_map_is_dragging(g_app.map.component_id);
    cc_clay_set_pointer(x, y, dragging);

    if (dragging) {
        double new_lat, new_lon;
        if (cc_map_pointer_move(g_app.map.component_id, g_app.map.zoom, x, y, &new_lat, &new_lon)) {
...
EXPORT void map_pointer_down(float x, float y) {
    cc_map_pointer_down(g_app.map.component_id, g_app.map.lat, g_app.map.lon, x, y);
    cc_clay_set_pointer(x, y, true);
}

EXPORT void map_pointer_up(float x, float y) {
    cc_map_pointer_up(g_app.map.component_id, x, y);
    cc_clay_set_pointer(x, y, false);
}

EXPORT void map_scroll(float delta, float x, float y) {
    (void)x; (void)y;
    g_app.map.zoom = cc_map_scroll(g_app.map.zoom, delta > 0 ? 1 : -1, 0, 19);
}

EXPORT int map_handle_click(float x, float y) {
    (void)x; (void)y;

    bool on_ui = cc_clay_pointer_over("InfoPanel") ||
                 cc_clay_pointer_over("LayerPanel") ||
                 cc_clay_pointer_over("ZoomControls") ||
                 cc_clay_pointer_over("TileInfo") ||
                 cc_clay_pointer_over("Attribution");

    if (cc_focused_id() != 0 && !on_ui) {
        cc_blur();
    }
...
EXPORT int map_frame(float dt) {
    if (!cc_clay_is_initialized()) return -1;

    cc_clay_begin_frame();
    render_ui();
    return cc_clay_end_frame(dt);
}
