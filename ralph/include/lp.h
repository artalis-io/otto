...
#define RALPH_FORCE_REFACTOR_PIVOT_TOL 1e-5
...
/* The telemetry snapshot types live in lp_telemetry.h. Nothing here
 * embeds them, so this header does not include that one: a file that
 * wants a snapshot asks for it. */
...
/* The lp_telemetry_* declarations live in lp_telemetry.h, with the snapshot
 * types they fill. 217 of the 313 functions this header declared were
 * telemetry; the solver interface underneath is the rest. Nothing here needs
 * them, so this header does not include that one. */
