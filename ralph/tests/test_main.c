...
/* ============================================================================
 * Test: Pseudo-Cost Update (Phase 1A+1C)
 *
 * Verify that pseudo-costs are actually updated during B&B.
 * We create a small MIP and solve it with VAR_SELECT_PSEUDO_COST.
 * Since pseudo-costs are now updated from actual branching outcomes,
 * the solver should make better branching decisions and explore fewer nodes
 * than a most-infeasible strategy on harder instances.
 * ============================================================================ */
void test_pseudo_cost_update(void) {
    printf("\n=== Test: Pseudo-Cost Update ===\n");

    /* Small MIP: 4 binary vars, 4 constraints, known optimal = 11 */
    RalphModel *model = ralph_create();
    ralph_set_obj_sense(model, RALPH_MINIMIZE);
    ralph_set_int_param(model, "verbose", 0);

    /* x0, x1, x2, x3 — binary with costs 5, 3, 4, 6 */
    ...
    /* x0 + x1 + x2 >= 2 */
    { int idx[] = {0, 1, 2}; double v[] = {1, 1, 1};
      ralph_add_constraint(model, 3, idx, v, 'G', 2.0); }

    /* x1 + x3 >= 1 */
    { int idx[] = {1, 3}; double v[] = {1, 1};
      ralph_add_constraint(model, 2, idx, v, 'G', 1.0); }

    /* x0 + x2 + x3 >= 1 */
    { int idx[] = {0, 2, 3}; double v[] = {1, 1, 1};
      ralph_add_constraint(model, 3, idx, v, 'G', 1.0); }

    /* x0 + x1 + x2 + x3 <= 3 */
    { int idx[] = {0, 1, 2, 3}; double v[] = {1, 1, 1, 1};
      ralph_add_constraint(model, 4, idx, v, 'L', 3.0); }
    ...
    double obj = ralph_get_objval(model);
    /* Optimal: x1=1, x2=1 → obj = 3+4 = 7, or x1=1, x0=1 → 3+5 = 8, etc.
     * Need to verify the actual optimum. With the constraints:
     * - Need at least 2 of {x0,x1,x2}
     * - Need at least 1 of {x1,x3}
     * - Need at least 1 of {x0,x2,x3}
     * Best: x1=1, x2=1 satisfies all three (coverage: {0,1,2}>=2, {1,3}>=1, {0,2,3}>=1)
     * Obj = 3+4 = 7 */
    ASSERT_NEAR(obj, 7.0, TOLERANCE, "Optimal objective");

    int nodes = ralph_get_node_count(model);
    ASSERT(nodes >= 0, "Node count is non-negative");
    printf("  INFO: Nodes explored: %d\n", nodes);

    ralph_free(model);
}

/* ============================================================================
 * Test: Pseudo-Cost Obj Init (Phase 1B)
 *
 * Verify that objective-coefficient initialization produces correct solutions
 * on a MIP with diverse coefficients. Also verify SCP init still works.
 * ============================================================================ */
void test_pseudo_cost_obj_init(void) {
    printf("\n=== Test: Pseudo-Cost Obj Init ===\n");

    /* MIP with diverse coefficients: c = {0.1, 10, 100, 1} */
    RalphModel *model = ralph_create();
    ralph_set_obj_sense(model, RALPH_MINIMIZE);
    ralph_set_int_param(model, "verbose", 0);

    ralph_add_var(model, 0.0, 1.0, 0.1, RALPH_BINARY);
    ralph_add_var(model, 0.0, 1.0, 10.0, RALPH_BINARY);
    ralph_add_var(model, 0.0, 1.0, 100.0, RALPH_BINARY);
    ralph_add_var(model, 0.0, 1.0, 1.0, RALPH_BINARY);
    ...
    double obj = ralph_get_objval(model);
    /* Optimal: x0=1, x3=1 → obj = 0.1 + 1.0 = 1.1 */
    ASSERT_NEAR(obj, 1.1, TOLERANCE, "Optimal with diverse coefficients");

    /* Verify SCP init works on an SCP-structured problem */
    RalphModel *scp = ralph_create();
    ralph_set_obj_sense(scp, RALPH_MINIMIZE);
    ralph_set_int_param(scp, "verbose", 0);

    /* Simple SCP: min 2x0 + 3x1 + 1x2 s.t. covering constraints */
    ralph_add_var(scp, 0.0, 1.0, 2.0, RALPH_BINARY);
    ralph_add_var(scp, 0.0, 1.0, 3.0, RALPH_BINARY);
    ralph_add_var(scp, 0.0, 1.0, 1.0, RALPH_BINARY);

    /* x0 + x1 >= 1 (element 0 covered by sets 0,1) */
    { int idx[] = {0, 1}; double v[] = {1, 1};
      ralph_add_constraint(scp, 2, idx, v, 'G', 1.0); }

    /* x1 + x2 >= 1 (element 1 covered by sets 1,2) */
    { int idx[] = {1, 2}; double v[] = {1, 1};
      ralph_add_constraint(scp, 2, idx, v, 'G', 1.0); }

    ralph_optimize(scp);
    ASSERT(ralph_get_status(scp) == RALPH_STATUS_OPTIMAL, "SCP: Status is OPTIMAL");

    double scp_obj = ralph_get_objval(scp);
    /* Optimal: x0=1, x2=1 → obj = 2+1 = 3, or x1=1 → obj = 3 */
    ASSERT(scp_obj <= 3.0 + TOLERANCE, "SCP: Objective <= 3");
...
/* ============================================================================
 * Test: Root Strong Branching (Phase 2)
 *
 * Compare MIP solving with root strong branching on/off.
 * Both should produce correct optimal solutions. With root SB, the solver
 * should explore fewer nodes on problems with misleading objective coefficients.
 * ============================================================================ */
void test_root_strong_branching(void) {
    printf("\n=== Test: Root Strong Branching ===\n");

    /* Create a MIP where objective coefficients are misleading.
     * Variables with small obj coefficients cause large LP changes when branched. */
    int nodes_on, nodes_off;
    double obj_on, obj_off;

    for (int use_rsb = 0; use_rsb <= 1; use_rsb++) {
        RalphModel *m = ralph_create();
        ralph_set_obj_sense(m, RALPH_MINIMIZE);
        ralph_set_int_param(m, "verbose", 0);
        ralph_set_int_param(m, "root_strong_branch", use_rsb);

        /* 8 binary vars with misleading costs */
        double costs[] = {1.0, 1.0, 1.0, 1.0, 100.0, 100.0, 100.0, 100.0};
        for (int j = 0; j < 8; j++) {
            ralph_add_var(m, 0.0, 1.0, costs[j], RALPH_BINARY);
        }

        /* Covering constraints that interact tightly */
        { int idx[] = {0, 1, 4}; double v[] = {1, 1, 1};
          ralph_add_constraint(m, 3, idx, v, 'G', 1.0); }
        { int idx[] = {1, 2, 5}; double v[] = {1, 1, 1};
          ralph_add_constraint(m, 3, idx, v, 'G', 1.0); }
        { int idx[] = {2, 3, 6}; double v[] = {1, 1, 1};
          ralph_add_constraint(m, 3, idx, v, 'G', 1.0); }
        { int idx[] = {3, 0, 7}; double v[] = {1, 1, 1};
          ralph_add_constraint(m, 3, idx, v, 'G', 1.0); }
        /* Sum of expensive vars <= 1 */
        { int idx[] = {4, 5, 6, 7}; double v[] = {1, 1, 1, 1};
          ralph_add_constraint(m, 4, idx, v, 'L', 1.0); }

        ralph_optimize(m);
        ASSERT(ralph_get_status(m) == RALPH_STATUS_OPTIMAL,
               use_rsb ? "RSB=on: Status OPTIMAL" : "RSB=off: Status OPTIMAL");

        double obj = ralph_get_objval(m);
        int nodes = ralph_get_node_count(m);
        ...
        printf("  INFO: RSB=%d: obj=%.2f, nodes=%d\n", use_rsb, obj, nodes);
        ralph_free(m);
    }

    /* Both must find the same optimal */
    ASSERT_NEAR(obj_on, obj_off, TOLERANCE, "RSB on/off: same optimal");

    /* Correctness: optimal should select cheap vars (obj = 2 or 3) */
    ASSERT(obj_on <= 4.0 + TOLERANCE, "Optimal uses cheap variables");
    (void)nodes_on; (void)nodes_off; /* Used only in printf */
}

/* ============================================================================
 * Test: Node Probing (Phase 4)
 *
 * Create a MIP where probing at depth 1 can detect infeasible directions.
 * Compare node counts with probing on vs off — probing should reduce nodes.
 * ============================================================================ */
void test_node_probing(void) {
    printf("\n=== Test: Node Probing ===\n");

    int nodes_on, nodes_off;
    double obj_on, obj_off;

    for (int use_probing = 0; use_probing <= 1; use_probing++) {
        RalphModel *m = ralph_create();
        ralph_set_obj_sense(m, RALPH_MINIMIZE);
        ralph_set_int_param(m, "verbose", 0);
        ralph_set_int_param(m, "probing_at_nodes", use_probing);

        /* 6 binary vars */
        double costs[] = {10.0, 8.0, 6.0, 4.0, 12.0, 3.0};
        for (int j = 0; j < 6; j++) {
            ralph_add_var(m, 0.0, 1.0, costs[j], RALPH_BINARY);
        }

        /* Tight constraints where probing reveals fixings */
        /* x0 + x1 >= 1 */
        { int idx[] = {0, 1}; double v[] = {1, 1};
          ralph_add_constraint(m, 2, idx, v, 'G', 1.0); }
        /* x2 + x3 >= 1 */
        { int idx[] = {2, 3}; double v[] = {1, 1};
          ralph_add_constraint(m, 2, idx, v, 'G', 1.0); }
        /* x4 + x5 >= 1 */
        { int idx[] = {4, 5}; double v[] = {1, 1};
          ralph_add_constraint(m, 2, idx, v, 'G', 1.0); }
        /* x0 + x2 + x4 <= 2 */
        { int idx[] = {0, 2, 4}; double v[] = {1, 1, 1};
          ralph_add_constraint(m, 3, idx, v, 'L', 2.0); }
        /* x1 + x3 + x5 <= 2 */
        { int idx[] = {1, 3, 5}; double v[] = {1, 1, 1};
          ralph_add_constraint(m, 3, idx, v, 'L', 2.0); }

        ralph_optimize(m);
        ASSERT(ralph_get_status(m) == RALPH_STATUS_OPTIMAL,
               use_probing ? "Probing=on: Status OPTIMAL" : "Probing=off: Status OPTIMAL");
        ...
        printf("  INFO: Probing=%d: obj=%.2f, nodes=%d\n", use_probing, obj, nodes);
        ralph_free(m);
    }

    /* Both must find the same optimal */
    ASSERT_NEAR(obj_on, obj_off, TOLERANCE, "Probing on/off: same optimal");

    /* Optimal: x0=1 (10), x3=1 (4), x5=1 (3) → obj = 17
     * (x1+x3+x5=3 violates x1+x3+x5<=2, so the greedy x1,x3,x5 is infeasible) */
    ASSERT(obj_on <= 17.0 + TOLERANCE, "Optimal objective <= 17");
    (void)nodes_on; (void)nodes_off; /* Used only in printf */
}
...
int main(int argc, char **argv) {
    ...
    if (!skip_mip) {
        ...
        /* Pseudo-cost and probing tests */
        test_pseudo_cost_update();
        test_pseudo_cost_obj_init();
        test_root_strong_branching();
        test_node_probing();
    } else {
        printf("\n=== MIP tests skipped ===\n");
    }
