...
int main(void) {
    ...
    ralph_test_add_var(model, 0.0, 1e30, 1.0, 'C');  /* x1 */
    ralph_test_add_var(model, 0.0, 1e30, 1.0, 'C');  /* x2 */

    int idx[] = {0, 1};
    double val1[] = {1.0, 1.0};
    ralph_test_add_constraint(model, 2, idx, val1, 'L', 4.0);  /* x1 + x2 <= 4 */
    double val2[] = {2.0, 1.0};
    ralph_test_add_constraint(model, 2, idx, val2, 'L', 6.0);  /* 2x1 + x2 <= 6 */
