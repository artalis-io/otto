...
static void test_runtime_telemetry_gate(void) {
...

    {
        ...
        solver.telemetry.perf_pricing_ms = 3.0;
        solver.telemetry.perf_ratio_ms = 2.0;
        solver.telemetry.perf_refactor_count = 5;
        ...
        ASSERT_DBL_EQ(solver.telemetry.perf_pricing_ms, 3.0, "gate off: pricing unchanged");
        ASSERT_DBL_EQ(solver.telemetry.perf_ratio_ms, 2.0, "gate off: ratio unchanged");
        ASSERT_INT_EQ(solver.telemetry.perf_refactor_count, 5, "gate off: refactor count unchanged");
...

    {
        ...
        lu.telemetry.perf_symbolic_calls = 4;
        lu.telemetry.mkz_calls = 7;
        lu.telemetry.identity_sep_failures = 2;
        ...
        ASSERT_INT_EQ(lu.telemetry.perf_symbolic_calls, 4, "gate off: LU symbolic calls unchanged");
        ASSERT_INT_EQ(lu.telemetry.mkz_calls, 7, "gate off: LU mkz calls unchanged");
        ASSERT_INT_EQ(lu.telemetry.identity_sep_failures, 2, "gate off: LU identity failures unchanged");
