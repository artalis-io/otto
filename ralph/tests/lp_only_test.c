...
int main(void) {
    ...
    printf("Test 1: Simple 2-var LP\n");
    {
        ...
        printf("  Status: %d, Obj: %.2f\n", ralph_test_get_status(model), ralph_test_get_objval(model));
    ...
    printf("Test 2: Equality constraint\n");
    {
        RalphModel *model = ralph_test_create();
        ralph_test_set_obj_sense(model, RALPH_MINIMIZE);
        ralph_test_add_var(model, 1.0, 1e30, 1.0, RALPH_CONTINUOUS);
        ralph_test_add_var(model, 0.0, 1e30, 2.0, RALPH_CONTINUOUS);
        int idx[] = {0, 1};
        double val[] = {1.0, 1.0};
