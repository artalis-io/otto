...
static int find_var(const RalphModel *model, const char *name) {
    ...
    for (int i = 0; i < n; i++) {
        const char *v = ralph_get_var_name(model, i);
...
static void test_forplan_embedded_space_names(void) {
    ...
    int rc = ralph_read_mps(model, "benchmarks/netlib/forplan.mps");
    TEST(rc == 0, "forplan parsed successfully");
    if (rc == 0) {
        int nvars = ralph_get_num_vars(model);
        int ncons = ralph_get_num_cons(model);
        ...
        if (idx >= 0) {
            ...
            int brc = ralph_get_var_bounds(model, idx, &lb, &ub);
...
static void test_sierra_omitted_bound_set_name(void) {
    ...
    int rc = ralph_read_mps(model, "benchmarks/netlib/sierra.mps");
...
static void test_e226_objective_constant_from_rhs(void) {
    ...
    int rc = ralph_read_mps(model, "benchmarks/netlib/e226.mps");
    TEST(rc == 0, "e226 parsed successfully");
    if (rc == 0) {
        rc = ralph_optimize(model);
        TEST(rc == 0, "e226 optimize succeeds");
        if (rc == 0) {
            TEST(ralph_get_status(model) == RALPH_STATUS_OPTIMAL, "e226 status optimal");
            TEST(fabs(ralph_get_objval(model) - (-25.86492907)) < 1e-4,
