...
RalphLPModel* ralph_lp_create(void) {
    return (RalphLPModel *)ralph_core_create();
}
...
int ralph_lp_set_obj_sense(RalphLPModel *model, RalphLPObjSense sense) {
    return ralph_core_set_obj_sense((RalphModel *)model, (RalphObjSense)sense);
}

int ralph_lp_add_var(RalphLPModel *model, double lb, double ub, double obj, RalphLPVarType type) {
    return ralph_core_add_var((RalphModel *)model, lb, ub, obj, (RalphVarType)type);
}
...
int ralph_lp_add_vars(RalphLPModel *model, int count, const double *lb, const double *ub,
    ...
    ret = ralph_core_add_vars((RalphModel *)model, count, lb, ub, obj, types_conv);
    free(types_conv);
    return ret;
}
...
int ralph_lp_add_constraint(RalphLPModel *model, int nnz, const int *indices,
    ...
    return ralph_core_add_constraint((RalphModel *)model, nnz, indices, values, (RalphSense)sense, rhs);
}

int ralph_lp_set_var_bounds(RalphLPModel *model, int var, double lb, double ub) {
    return ralph_core_set_var_bounds((RalphModel *)model, var, lb, ub);
}

int ralph_lp_set_var_type(RalphLPModel *model, int var, RalphLPVarType type) {
    return ralph_core_set_var_type((RalphModel *)model, var, (RalphVarType)type);
}

int ralph_lp_set_obj_coef(RalphLPModel *model, int var, double coef) {
    return ralph_core_set_obj_coef((RalphModel *)model, var, coef);
}

int ralph_lp_set_obj_offset(RalphLPModel *model, double offset) {
    return ralph_core_set_obj_offset((RalphModel *)model, offset);
}

double ralph_lp_get_obj_offset(const RalphLPModel *model) {
    return ralph_core_get_obj_offset((const RalphModel *)model);
}

int ralph_lp_set_var_name(RalphLPModel *model, int var, const char *name) {
    return ralph_core_set_var_name((RalphModel *)model, var, name);
}

int ralph_lp_set_con_name(RalphLPModel *model, int con, const char *name) {
    return ralph_core_set_con_name((RalphModel *)model, con, name);
}

const char* ralph_lp_get_var_name(const RalphLPModel *model, int var) {
    return ralph_core_get_var_name((const RalphModel *)model, var);
}

const char* ralph_lp_get_con_name(const RalphLPModel *model, int con) {
    return ralph_core_get_con_name((const RalphModel *)model, con);
}

int ralph_lp_get_num_vars(const RalphLPModel *model) {
    return ralph_core_get_num_vars((const RalphModel *)model);
}

int ralph_lp_get_num_cons(const RalphLPModel *model) {
    return ralph_core_get_num_cons((const RalphModel *)model);
}

int ralph_lp_get_num_integer_vars(const RalphLPModel *model) {
    return ralph_core_get_num_integers((const RalphModel *)model);
}

int ralph_lp_get_var_bounds(const RalphLPModel *model, int var, double *lb, double *ub) {
    return ralph_core_get_var_bounds((const RalphModel *)model, var, lb, ub);
}
...
int ralph_lp_get_dual_solution(const RalphLPModel *model, double *y) {
    return ralph_core_get_dual_solution((const RalphModel *)model, y);
}

int ralph_lp_get_reduced_costs(const RalphLPModel *model, double *rc) {
    return ralph_core_get_reduced_costs((const RalphModel *)model, rc);
}

int ralph_lp_get_iterations(const RalphLPModel *model) {
    return ralph_core_get_iterations((const RalphModel *)model);
}

RalphLPBasis* ralph_lp_save_basis(const RalphLPModel *model) {
    return (RalphLPBasis *)ralph_core_save_basis((const RalphModel *)model);
}

int ralph_lp_load_basis(RalphLPModel *model, const RalphLPBasis *basis) {
    return ralph_core_load_basis((RalphModel *)model, (const RalphBasis *)basis);
}
...
int ralph_lp_get_basis_status(const RalphLPModel *model,
    ...
    m = ralph_core_get_num_cons((const RalphModel *)model);
    if (n < 0 || m < 0) return -1;
    ...
    ret = ralph_core_get_basis_status((const RalphModel *)model, tmp_col, tmp_row);
    if (ret == 0) {
        if (col_status) {
...
int ralph_lp_set_basis_status(RalphLPModel *model,
    ...
    ret = ralph_core_set_basis_status((RalphModel *)model, tmp_col, tmp_row);
    free(tmp_col);
    free(tmp_row);
    return ret;
}
