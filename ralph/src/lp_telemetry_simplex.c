...
void lp_telemetry_record_phase1_force_extreme_followup_stabilized(
    SimplexSolver *solver) {
    if (!solver_telemetry_enabled(solver)) return;
    solver->telemetry.perf_phase1_force_extreme_followup_stabilized++;
}

void lp_telemetry_record_phase1_force_extreme_followup_ratio_breakdown(
    SimplexSolver *solver) {
    if (!solver_telemetry_enabled(solver)) return;
    solver->telemetry.perf_phase1_force_extreme_followup_ratio_breakdown++;
}

void lp_telemetry_record_phase1_force_extreme_followup_failed_stabilize(
    SimplexSolver *solver) {
    if (!solver_telemetry_enabled(solver)) return;
    solver->telemetry.perf_phase1_force_extreme_followup_failed_stabilize++;
}

void lp_telemetry_record_phase1_force_extreme_followup_post_dir_skip_retry(
    SimplexSolver *solver) {
    if (!solver_telemetry_enabled(solver)) return;
    solver->telemetry.perf_phase1_force_extreme_followup_post_dir_skip_retry++;
}

void lp_telemetry_record_phase1_force_extreme_followup_post_dir_skip_dual_rescue(
    SimplexSolver *solver) {
    if (!solver_telemetry_enabled(solver)) return;
    solver->telemetry.perf_phase1_force_extreme_followup_post_dir_skip_dual_rescue++;
}

void lp_telemetry_record_phase1_force_extreme_followup_post_dir_skip_forced_refactor(
    SimplexSolver *solver) {
    if (!solver_telemetry_enabled(solver)) return;
    solver->telemetry.perf_phase1_force_extreme_followup_post_dir_skip_forced_refactor++;
}

void lp_telemetry_record_phase1_force_extreme_followup_next_failed_stabilize(
    SimplexSolver *solver) {
    if (!solver_telemetry_enabled(solver)) return;
    solver->telemetry.perf_phase1_force_extreme_followup_next_failed_stabilize++;
}

void lp_telemetry_record_phase1_force_extreme_followup_next_ratio_breakdown(
    SimplexSolver *solver) {
    if (!solver_telemetry_enabled(solver)) return;
    solver->telemetry.perf_phase1_force_extreme_followup_next_ratio_breakdown++;
}

void lp_telemetry_record_phase1_force_extreme_followup_next_pivot_fail(
    SimplexSolver *solver) {
    if (!solver_telemetry_enabled(solver)) return;
    solver->telemetry.perf_phase1_force_extreme_followup_next_pivot_fail++;
}

void lp_telemetry_record_phase1_force_extreme_followup_next_pivot_success(
    SimplexSolver *solver) {
    if (!solver_telemetry_enabled(solver)) return;
    solver->telemetry.perf_phase1_force_extreme_followup_next_pivot_success++;
}
...
void lp_telemetry_record_phase1_force_extreme_followup_direction(
    ...
    solver->telemetry.perf_phase1_force_extreme_followup_dir_samples++;

    if (dir_nnz > 0) {
        solver->telemetry.perf_phase1_force_extreme_followup_dir_nnz_total +=
            dir_nnz;
        if (dir_nnz >
            solver->telemetry.perf_phase1_force_extreme_followup_dir_nnz_max) {
            solver->telemetry.perf_phase1_force_extreme_followup_dir_nnz_max =
                dir_nnz;
        }
    }
    if (isfinite(dir_inf) && dir_inf >= 0.0) {
        solver->telemetry.perf_phase1_force_extreme_followup_dir_inf_total +=
            dir_inf;
        if (dir_inf >
            solver->telemetry.perf_phase1_force_extreme_followup_dir_inf_max) {
            solver->telemetry.perf_phase1_force_extreme_followup_dir_inf_max =
                dir_inf;
        }
    }
    if (isfinite(pivot_abs) && pivot_abs >= 0.0) {
        solver->telemetry.perf_phase1_force_extreme_followup_pivot_abs_total +=
            pivot_abs;
        if (pivot_abs >
            solver->telemetry.perf_phase1_force_extreme_followup_pivot_abs_max) {
            solver->telemetry.perf_phase1_force_extreme_followup_pivot_abs_max =
                pivot_abs;
        }
    }
    if (isfinite(theta) && theta >= 0.0) {
        solver->telemetry.perf_phase1_force_extreme_followup_theta_total +=
            theta;
        if (theta >
            solver->telemetry.perf_phase1_force_extreme_followup_theta_max) {
            solver->telemetry.perf_phase1_force_extreme_followup_theta_max =
                theta;
        }
    }
    ...
    if (leaving == -2) {
        solver->telemetry.perf_phase1_force_extreme_followup_dir_bound_flip++;
        bound_geometry = 1;
    }
    if (isfinite(theta) && theta <= RALPH_FEAS_TOL) {
        solver->telemetry.perf_phase1_force_extreme_followup_dir_tiny_theta++;
        bound_geometry = 1;
    }
    if (bound_geometry) {
        solver->telemetry.perf_phase1_force_extreme_followup_dir_bound_geometry++;
        return;
    }
    ...
    if (pivot_ratio <= 1e-6) {
        solver->telemetry.perf_phase1_force_extreme_followup_dir_weak_leaving++;
    } else {
        solver->telemetry.perf_phase1_force_extreme_followup_dir_ftran_shape++;
    }
}
...
void lp_telemetry_record_phase1_force_extreme_tiny_theta_relax(
    SimplexSolver *solver) {
    if (!solver_telemetry_enabled(solver)) return;
    solver->telemetry.perf_phase1_force_extreme_tiny_theta_relax_applied++;
}

enum {
    LP_PHASE1_FORCE_EXTREME_TINY_THETA_RELAX_REFACTOR_LU_HEALTH = 1,
    LP_PHASE1_FORCE_EXTREME_TINY_THETA_RELAX_REFACTOR_FORCE_PIVOT = 2,
    LP_PHASE1_FORCE_EXTREME_TINY_THETA_RELAX_REFACTOR_LADDER = 3
};

void lp_telemetry_record_phase1_force_extreme_tiny_theta_relax_post_dir_skip_retry(
    SimplexSolver *solver) {
    if (!solver_telemetry_enabled(solver)) return;
    solver->telemetry.perf_phase1_force_extreme_tiny_theta_relax_post_dir_skip_retry++;
}

void lp_telemetry_record_phase1_force_extreme_tiny_theta_relax_post_dir_skip_dual_rescue(
    SimplexSolver *solver) {
    if (!solver_telemetry_enabled(solver)) return;
    solver->telemetry.perf_phase1_force_extreme_tiny_theta_relax_post_dir_skip_dual_rescue++;
}

void lp_telemetry_record_phase1_force_extreme_tiny_theta_relax_post_dir_skip_forced_refactor(
    SimplexSolver *solver) {
    if (!solver_telemetry_enabled(solver)) return;
    solver->telemetry.perf_phase1_force_extreme_tiny_theta_relax_post_dir_skip_forced_refactor++;
}
...
void lp_telemetry_record_phase1_force_extreme_tiny_theta_relax_refactor(
    ...
    if (!solver_telemetry_enabled(solver)) return;
    switch (reason) {
        case LP_PHASE1_FORCE_EXTREME_TINY_THETA_RELAX_REFACTOR_LU_HEALTH:
            solver->telemetry.perf_phase1_force_extreme_tiny_theta_relax_refactor_force_lu_health++;
            break;
        case LP_PHASE1_FORCE_EXTREME_TINY_THETA_RELAX_REFACTOR_FORCE_PIVOT:
            solver->telemetry.perf_phase1_force_extreme_tiny_theta_relax_refactor_force_pivot_mode++;
            break;
        case LP_PHASE1_FORCE_EXTREME_TINY_THETA_RELAX_REFACTOR_LADDER:
            solver->telemetry.perf_phase1_force_extreme_tiny_theta_relax_refactor_ladder_force++;
            break;
        default:
            break;
    }
...
void lp_telemetry_record_phase1_force_extreme_tiny_theta_relax_next_failed_stabilize(
    SimplexSolver *solver) {
    if (!solver_telemetry_enabled(solver)) return;
    solver->telemetry.perf_phase1_force_extreme_tiny_theta_relax_next_failed_stabilize++;
}

void lp_telemetry_record_phase1_force_extreme_tiny_theta_relax_next_ratio_breakdown(
    SimplexSolver *solver) {
    if (!solver_telemetry_enabled(solver)) return;
    solver->telemetry.perf_phase1_force_extreme_tiny_theta_relax_next_ratio_breakdown++;
}

void lp_telemetry_record_phase1_force_extreme_tiny_theta_relax_next_pivot_fail(
    SimplexSolver *solver) {
    if (!solver_telemetry_enabled(solver)) return;
    solver->telemetry.perf_phase1_force_extreme_tiny_theta_relax_next_pivot_fail++;
}

void lp_telemetry_record_phase1_force_extreme_tiny_theta_relax_next_pivot_success(
    SimplexSolver *solver) {
    if (!solver_telemetry_enabled(solver)) return;
    solver->telemetry.perf_phase1_force_extreme_tiny_theta_relax_next_pivot_success++;
}
