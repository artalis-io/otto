...
void update_pseudo_costs(MIPSolver *solver, int var, double val,
    ...
    if (dir == BRANCH_DOWN) {
        ...
        if (delta_x > RALPH_ZERO_TOL) {
            ...
            if (new_pseudo > MIP_PCOST_MAX_VALUE) new_pseudo = MIP_PCOST_MAX_VALUE;
...
/* ============================================================================
 * Node Probing (Lightweight Bound Propagation at B&B Nodes)
 *
 * For binary variables near 0.5 in the LP solution, temporarily fix to 0 or 1
 * and propagate implied bounds. If one direction is infeasible, fix the variable.
 * If both are feasible, intersect the implied bounds for global tightening.
 *
 * This is much cheaper than full presolve probing: we only propagate from
 * a single constraint at a time (column-based), not all constraints.
 * ============================================================================ */

/*
 * Lightweight single-variable bound propagation.
 *
 * Fix variable j to fix_val, then for each constraint containing j,
 * compute implied bounds on other variables using row activity analysis.
 *
 * Parameters:
 *   solver    - MIP solver (for model access)
 *   j         - Variable index to fix
 *   fix_val   - Value to fix to (0.0 or 1.0)
 *   impl_lb   - Output: implied lower bounds (caller-allocated, size num_vars)
 *   impl_ub   - Output: implied upper bounds (caller-allocated, size num_vars)
 *   node_lb   - Current node lower bounds
 *   node_ub   - Current node upper bounds
 *
 * Returns:
 *   0 if feasible, 1 if infeasible (constraint violated)
 */
static int propagate_variable_fixing(MIPSolver *solver, int j, double fix_val,
                                      double *impl_lb, double *impl_ub,
                                      const double *node_lb, const double *node_ub) {
    LPModel *model = solver->working_model;
    SparseMatrix *A = model->A;
    int n = model->num_vars;

    /* Start with current node bounds */
    memcpy(impl_lb, node_lb, n * sizeof(double));
    memcpy(impl_ub, node_ub, n * sizeof(double));
    impl_lb[j] = fix_val;
    impl_ub[j] = fix_val;

    /* Iterate over constraints containing variable j (column j of A) */
    for (int p = A->colptr[j]; p < A->colptr[j + 1]; p++) {
        int row = A->rowidx[p];
        ...
        char sense = model->sense[row];

        /* Extract dense row and compute activity bounds */
        double *row_vals = (double *)calloc(n, sizeof(double));
        if (!row_vals) continue;
        sparse_get_row(A, row, row_vals);

        /* Compute activity bounds of other variables in this row */
        double other_lb = 0.0, other_ub = 0.0;
        double abs_sum = 0.0;
        int all_finite = 1;
        for (int k = 0; k < n; k++) {
            double akr = row_vals[k];
            if (fabs(akr) < RALPH_ZERO_TOL) continue;
            if (k == j) {
                /* Fixed variable contribution */
                other_lb += akr * fix_val;
                other_ub += akr * fix_val;
                abs_sum += fabs(akr * fix_val);
                continue;
            }
            abs_sum += fabs(akr) * fmax(fabs(impl_lb[k]), fabs(impl_ub[k]));
            if (akr > 0) {
                if (impl_lb[k] <= -RALPH_INFINITY / 2) { all_finite = 0; continue; }
                other_lb += akr * impl_lb[k];
                if (impl_ub[k] >= RALPH_INFINITY / 2) { all_finite = 0; continue; }
                other_ub += akr * impl_ub[k];
            } else {
                if (impl_ub[k] >= RALPH_INFINITY / 2) { all_finite = 0; continue; }
                other_lb += akr * impl_ub[k];
                if (impl_lb[k] <= -RALPH_INFINITY / 2) { all_finite = 0; continue; }
                other_ub += akr * impl_lb[k];
            }
        }
        ...
        /* Check feasibility: is row activity compatible with RHS? */
        if (sense == 'L' && other_lb > rhs + RALPH_FEAS_TOL) {
            free(row_vals);
            return 1; /* Infeasible */
        }
        if (sense == 'G' && other_ub < rhs - RALPH_FEAS_TOL) {
            free(row_vals);
            return 1; /* Infeasible */
        }
        if (sense == 'E' && (other_lb > rhs + RALPH_FEAS_TOL || other_ub < rhs - RALPH_FEAS_TOL)) {
            free(row_vals);
            return 1; /* Infeasible */
        }

        /* Derive implied bounds for each variable in this row */
        for (int k = 0; k < n; k++) {
            if (k == j) continue;
            double akr = row_vals[k];
            if (fabs(akr) < RALPH_ZERO_TOL) continue;

            /* Cancellation guard: skip if variable's contribution dominates */
            double k_contrib = fabs(akr) * fmax(fabs(impl_lb[k]), fabs(impl_ub[k]));
            if (abs_sum > RALPH_ZERO_TOL && k_contrib > 0.1 * abs_sum) continue;

            /* Compute other_without_k bounds */
            double rest_lb, rest_ub;
            if (akr > 0) {
                rest_lb = other_lb - akr * impl_lb[k];
                rest_ub = other_ub - akr * impl_ub[k];
            } else {
                rest_lb = other_lb - akr * impl_ub[k];
                rest_ub = other_ub - akr * impl_lb[k];
            }

            /* For <= constraint: rest + akr*xk <= rhs → xk <= (rhs - rest_lb) / akr */
            if (sense == 'L' || sense == 'E') {
                if (akr > RALPH_ZERO_TOL) {
                    double new_ub = (rhs - rest_lb) / akr;
                    if (new_ub < impl_ub[k] - RALPH_FEAS_TOL) impl_ub[k] = new_ub;
                } else if (akr < -RALPH_ZERO_TOL) {
                    double new_lb = (rhs - rest_lb) / akr;
                    if (new_lb > impl_lb[k] + RALPH_FEAS_TOL) impl_lb[k] = new_lb;
                }
            }
            /* For >= constraint: rest + akr*xk >= rhs → xk >= (rhs - rest_ub) / akr */
            if (sense == 'G' || sense == 'E') {
                if (akr > RALPH_ZERO_TOL) {
                    double new_lb = (rhs - rest_ub) / akr;
                    if (new_lb > impl_lb[k] + RALPH_FEAS_TOL) impl_lb[k] = new_lb;
                } else if (akr < -RALPH_ZERO_TOL) {
                    double new_ub = (rhs - rest_ub) / akr;
                    if (new_ub < impl_ub[k] - RALPH_FEAS_TOL) impl_ub[k] = new_ub;
                }
            }

            /* Check for infeasibility from tightened bounds */
            if (impl_lb[k] > impl_ub[k] + RALPH_FEAS_TOL) {
                free(row_vals);
                return 1; /* Infeasible */
            }
        }
...
/*
 * Probing bound tightening at B&B nodes.
 *
 * For fractional binary variables, probe both directions (fix to 0 and 1).
 * If one direction is infeasible, fix the variable. If both feasible,
 * intersect implied bounds.
 *
 * Returns: number of bounds tightened, or -1 if node is infeasible.
 */
int probing_bound_tightening(MIPSolver *solver) {
    ...
    /* Allocate implied bounds arrays */
    double *impl_lb0 = (double *)malloc(n * sizeof(double));
    double *impl_ub0 = (double *)malloc(n * sizeof(double));
    double *impl_lb1 = (double *)malloc(n * sizeof(double));
    double *impl_ub1 = (double *)malloc(n * sizeof(double));
    if (!impl_lb0 || !impl_ub0 || !impl_lb1 || !impl_ub1) {
        free(impl_lb0); free(impl_ub0); free(impl_lb1); free(impl_ub1);
        return 0;
    }

    int probed = 0;
    for (int k = 0; k < solver->num_integers && probed < MIP_PROBE_MAX_VARS; k++) {
        int j = solver->integer_vars[k];
        ...
        double val = sol[j];
        double frac = val - floor(val);
        if (frac < MIP_PROBE_FRAC_THRESHOLD ||
            frac > 1.0 - MIP_PROBE_FRAC_THRESHOLD) continue;

        probed++;

        /* Probe x_j = 0 */
        int infeas_0 = propagate_variable_fixing(solver, j, 0.0,
                                                  impl_lb0, impl_ub0,
                                                  model->lb, model->ub);

        /* Probe x_j = 1 */
        int infeas_1 = propagate_variable_fixing(solver, j, 1.0,
                                                  impl_lb1, impl_ub1,
                                                  model->lb, model->ub);

        if (infeas_0 && infeas_1) {
            /* Both directions infeasible — node is infeasible */
            free(impl_lb0); free(impl_ub0); free(impl_lb1); free(impl_ub1);
            return -1;
        }
        ...
        if (infeas_0) {
             ...
             * A bound valid in BOTH probes is globally valid.
             * global lb = min(lb_probe0, lb_probe1)
             * global ub = max(ub_probe0, ub_probe1) */
            for (int v = 0; v < n; v++) {
                if (v == j) continue;
                double new_lb = fmin(impl_lb0[v], impl_lb1[v]);
                double new_ub = fmax(impl_ub0[v], impl_ub1[v]);

                if (new_lb > model->lb[v] + RALPH_FEAS_TOL) {
                    model->lb[v] = new_lb;
                    count++;
                }
                if (new_ub < model->ub[v] - RALPH_FEAS_TOL) {
                    model->ub[v] = new_ub;
                    count++;
                }
            }
