...
/*
 * Append to a bounded buffer, tracking both the write position and the length
 * the output would have had.
 *
 * snprintf returns the second of those -- what it WOULD have written -- not
 * what it did. So the obvious `pos += snprintf(buf + pos, size - pos, ...)`
 * is a buffer overflow waiting for a buffer one byte too small: pos passes
 * size, the next `size - pos` underflows size_t to something near 2^64, and
 * snprintf is handed a pointer past the end of the buffer and told it has
 * eighteen exabytes of room. AddressSanitizer caught it as a
 * stack-buffer-overflow, WRITE of size 7, through the public
 * ralph_lp_write_solution_buf().
 *
 * `total` is what the caller needs to size a buffer; `pos` never leaves the
 * buffer. When there is no room left, vsnprintf is called with NULL and 0,
 * which is the defined way to ask for the length without writing.
 */
static void sol_appendf(char *buf, size_t buf_size, size_t *pos, size_t *total,
                        const char *fmt, ...)
{
    size_t space = (*pos < buf_size) ? buf_size - *pos : 0;
    va_list ap;
    int n;

    va_start(ap, fmt);
    n = vsnprintf(space ? buf + *pos : NULL, space, fmt, ap);
    va_end(ap);

    if (n < 0) return;                 /* encoding error: write nothing */
    *total += (size_t)n;
    if (space && (size_t)n < space) {
        *pos += (size_t)n;
    } else if (buf_size) {
        *pos = buf_size - 1;           /* full; stay inside the buffer */
    }
}
...
int ralph_core_write_solution_buf(const RalphLPModel *model, char *buf, size_t buf_size) {
    ...
    size_t pos = 0;      /* write position, always inside the buffer */
    size_t total = 0;    /* length the output would have had */

    /* Header comment */
    sol_appendf(buf, buf_size, &pos, &total,
    ...
    if (status == RALPH_LP_STATUS_OPTIMAL ||
        ...
        double *x = (double *)malloc((size_t)num_vars * sizeof(double));
        ...
        sol_appendf(buf, buf_size, &pos, &total, "\\ Problem is infeasible\n");
    } else if (status == RALPH_LP_STATUS_UNBOUNDED) {
        sol_appendf(buf, buf_size, &pos, &total, "\\ Problem is unbounded\n");
    }

    buf[(pos < buf_size) ? pos : buf_size - 1] = '\0';

    /* snprintf's contract: the length the output would have had, so a caller
     * that got a truncated result can allocate total + 1 and ask again. */
    return (int)total;
}
