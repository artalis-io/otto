 ...
 * ============================================================================
 *
 * Configuration is loaded from /demo-config.json (generated by start script).
 * Falls back to default localhost ports if config file is not available.
 */

const DEFAULT_CONFIG = {
    carta: 'http://localhost:8081',
    velo: 'http://localhost:8082',
    locus: 'http://localhost:8083'
};
...
async function loadServerConfig() {
    try {
        // Config is served from project root by the demo HTTP server
        const response = await fetch('/demo-config.json');
        if (response.ok) {
            const config = await response.json();
            SERVER_CONFIG = {
                carta: config.carta || DEFAULT_CONFIG.carta,
                velo: config.velo || DEFAULT_CONFIG.velo,
                locus: config.locus || DEFAULT_CONFIG.locus
            };
            console.log('Loaded server config:', SERVER_CONFIG);
        } else {
            console.log('No config file, using defaults:', DEFAULT_CONFIG);
        }
    } catch (err) {
        console.log('Config load failed, using defaults:', DEFAULT_CONFIG);
    }
...
async function main() {
    ...
    try {
        ...
        // Configure servers from loaded config
