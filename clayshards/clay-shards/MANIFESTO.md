...
for (int i = 0; i < 3; i++) {
    ...
    cs_button(CS_ID(id), labels[i], NULL);
}
```
...
Widgets maintain internal state in a **fixed-size hash table keyed by stable ID**. Capacity is configurable at compile time (`CS_WIDGET_STATE_CAPACITY`) to preserve deterministic memory usage.
...
State persists across frames and focus changes. Components access state internally via `cs_widget_state(id)`.
