...
### nexus does not build on Windows: POSIX regex

- **Severity**: Medium. nexus is the only OTTO library not in the top-level
  Makefile and not in CI, so nothing currently notices.
- **Status**: One of two blockers fixed. The remaining one is a design decision.

`nx_validate.c` and `nx_xform.c` use POSIX `<regex.h>` -- `regcomp`, `regexec`
with capture groups, `regfree`, `REG_EXTENDED` -- for schema validation rules
and transform matching. Windows has no POSIX regex in either CRT, MinGW does not
ship one, and MSVC has nothing equivalent.

That is not a shim. The options each cost something:

- **Vendor an ERE engine.** Consistent with how OTTO already treats miniz, Clay
  and Keel, and keeps the zero-external-dependency rule. But an extended regular
  expression engine with captures is a real piece of code, and it drives schema
  validation, so its correctness is load-bearing.
- **Link libgnurx or libsystre on Windows.** Cheapest, but it is an external
  dependency the manifesto rules out, and it does nothing for MSVC.
- **Compile the pattern rules out on Windows.** Cheapest of all and the worst:
  validation and transform rules that use patterns would silently stop applying,
  so a document that fails validation on Linux would pass on Windows.

Until this is decided, nexus builds and passes on Linux and macOS only.

**The other blocker, now fixed:** every `%z` in nexus was an error on MinGW,
because `__attribute__((format(printf, ...)))` selects Microsoft's printf
checker there and it has no `%z`. nexus is the only module with `-Werror`, which
is why it was the only one that failed rather than warned. `SH_PRINTF_ATTR` in
`shared/include/sh_attr.h` now selects `gnu_printf` on MinGW, which matches the
printf MinGW actually links.
